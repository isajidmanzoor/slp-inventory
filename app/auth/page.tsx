...
export default function AuthPage() {
  ...
  async function handleSignup() {
    ...
    if (phone && !isValidPhone(phone)) { showToast('Enter a valid Pakistan number: 03xxxxxxxxx', 'err'); return }
  ...
  async function handleSendOtp() {
    if (method === 'email') {
      ...
      if (!emailVal || !isValidEmail(emailVal)) {
        showToast('Enter a valid email address', 'err'); return
      }
      setLoading(true)
      try {
        ...
        showToast('Reset link sent to ' + emailVal)
        // For email, supabase sends a link, not a 6-digit OTP — inform user
        showToast('📧 Check your email for a reset link. Click it to set a new password.', 'ok')
        startCountdown(60)
      } catch (e: any) {
        showToast(friendlyError(e.message), 'err')
      }
      ...
      if (!phoneVal || !isValidPhone(phone)) {
        showToast('Enter a valid Pakistan number: 03xxxxxxxxx', 'err'); return
      }
      setLoading(true)
      try {
  ...
  function friendlyError(msg: string): string {
    ...
    if (msg.includes('Phone number'))              return 'Invalid phone number'
    if (msg.includes('rate limit'))                return 'Too many attempts — wait a minute'
    return msg
  }

  function reset(m: Mode) {
    setMode(m); setOtpDigits(['','','','','','']); setPassword(''); setConfirm('')
  }
  ...
  return (
    <div style={{
      minHeight: '100vh', background: 'linear-gradient(135deg,#EAF2FB 0%,#F5F4F0 60%,#E8F1FB 100%)',
      ...
      {toast && (
        <div className="fixed bottom-5 left-1/2 z-50 max-w-xs w-auto px-4 py-2.5 rounded-2xl text-sm font-medium shadow-xl flex items-center gap-2"
          style={{
            ...
            background: toast.type==='ok' ? '#1C1B19' : '#9B2B2B',
      ...
      <div style={{
        ...
        boxShadow: '0 8px 48px rgba(26,95,168,0.12)', overflow: 'hidden',
      }}>
        {/* Header band */}
          ...
          {isLogin && (
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <div style={{ display:'flex', background:'#F5F4F0', borderRadius:12, padding:3 }}>
                {(['email','phone'] as Method[]).map(m => (
                  <button key={m} onClick={() => setMethod(m)} style={{
                    flex:1, height:34, borderRadius:10, border:'none', cursor:'pointer', fontSize:13,
                    fontWeight:600, transition:'all .2s',
                    background: method===m ? 'white' : 'transparent',
                    color: method===m ? SLP_BLUE : '#9C9B97',
              ...
              {method === 'email'
                ...
                : <input type="tel" placeholder="Phone: 03xxxxxxxxx" value={phone}
              ...
              <button type="button" onClick={() => reset('forgot')}
                style={{ textAlign:'right', fontSize:12, color:SLP_BLUE, background:'none',
              ...
              <button onClick={handleLogin} disabled={loading} style={{
                ...
                background: loading ? '#7FA8D0' : SLP_BLUE, color:'white',
              ...
              <div style={{ textAlign:'center', fontSize:13, color:'#6B6A66' }}>
                ...
                <button onClick={() => reset('signup')} style={{
                  color:SLP_BLUE, fontWeight:600, background:'none', border:'none',
          ...
          {isSignup && (
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              ...
              <input type="email" placeholder="Email address *" value={email}
                onChange={e => setEmail(e.target.value)}
                className={inp} style={{ borderColor:'#E4E2DC' }}/>
              <input type="tel" placeholder="Phone: 03xxxxxxxxx (optional)" value={phone}
                onChange={e => setPhone(e.target.value)}
                className={inp} style={{ borderColor:'#E4E2DC' }}/>
              <div style={{ position:'relative' }}>
              ...
              <div style={{ position:'relative' }}>
                <input type={showCpw?'text':'password'} placeholder="Confirm password" value={confirm}
                  onChange={e => setConfirm(e.target.value)}
                  onKeyDown={e => e.key==='Enter' && handleSignup()}
                  className={inp} style={{
                    borderColor: confirm && confirm !== password ? '#E05A5A' : '#E4E2DC',
                    paddingRight:44,
                  }}/>
                <button onClick={() => setShowCpw(p=>!p)} style={{
                  position:'absolute', right:14, top:'50%', transform:'translateY(-50%)',
                  background:'none', border:'none', cursor:'pointer', color:'#9C9B97',
                }}>{showCpw ? <EyeOff size={16}/> : <Eye size={16}/>}</button>
              </div>
              {confirm && confirm !== password && (
                <div style={{ fontSize:12, color:'#E05A5A', display:'flex', gap:4, alignItems:'center' }}>
              ...
              <div style={{ textAlign:'center', fontSize:13, color:'#6B6A66' }}>
                ...
                <button onClick={() => reset('login')} style={{
                  color:SLP_BLUE, fontWeight:600, background:'none', border:'none', cursor:'pointer', padding:0,
                }}>Sign in</button>
              </div>
            </div>
          )}
          ...
          {isForgot && (
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <button onClick={() => reset('login')} style={{
                ...
                color:'#6B6A66', background:'none', border:'none', cursor:'pointer', padding:0,
              }}><ArrowLeft size={14}/> Back to sign in</button>
              ...
              <button onClick={handleSendOtp} disabled={loading || countdown > 0} style={{
                width:'100%', height:46, borderRadius:14, border:'none', cursor:'pointer',
                background: (loading || countdown>0) ? '#7FA8D0' : SLP_BLUE,
              ...
              {method === 'email' && (
                <p style={{ fontSize:12, color:'#9C9B97', textAlign:'center', lineHeight:1.5 }}>
                  A password reset link will be sent to your email.<br/>Click it to set a new password.
                </p>
              )}
            </div>
          )}
          ...
          {isOtp && (
            <div style={{ display:'flex', flexDirection:'column', gap:18, alignItems:'center' }}>
              <button onClick={() => reset('forgot')} style={{
                alignSelf:'flex-start', display:'flex', alignItems:'center', gap:6, fontSize:13,
                color:'#6B6A66', background:'none', border:'none', cursor:'pointer', padding:0,
              }}><ArrowLeft size={14}/> Back</button>

              <p style={{ fontSize:14, color:'#6B6A66', textAlign:'center' }}>
                Enter the 6-digit OTP sent to<br/>
                <strong style={{ color:'#1C1B19' }}>{otpTarget}</strong>
              </p>
                ...
                {otpDigits.map((d, i) => (
                  <input
                    ...
                    style={{
                      ...
                      borderRadius:12, border:`2px solid ${d ? SLP_BLUE : '#E4E2DC'}`,
                      outline:'none', background: d ? '#EAF2FB' : 'white',
                      color: SLP_BLUE, transition:'all .15s',
                    }}
                  />
                ))}
              </div>

              <button onClick={handleVerifyOtp} disabled={loading || otpDigits.join('').length !== 6} style={{
                width:'100%', height:46, borderRadius:14, border:'none', cursor:'pointer',
                background: (loading || otpDigits.join('').length !== 6) ? '#7FA8D0' : SLP_BLUE,
                color:'white', fontWeight:700, fontSize:15,
                display:'flex', alignItems:'center', justifyContent:'center', gap:8,
              }}>
                {loading && <RefreshCw size={16} className="animate-spin"/>}
              ...
              {/* Resend */}
              <button onClick={handleSendOtp} disabled={loading || countdown > 0} style={{
                fontSize:13, color: countdown > 0 ? '#9C9B97' : SLP_BLUE,
