...
async function getOgData(url: string) {
  try {
    ...
    return { title: get('og:title'), image: get('og:image'), price: getPrice() }
  } catch { return null }
}
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const existingMap = new Map(existing?.map(p => [p.store_url, p]) ?? [])

    for (const url of urls) {
      try {
        const og = await getOgData(url)
        if (!og?.title) { errors++; continue }
        ...
        if (found) {
          ...
          if (og.image && !found.image_url?.startsWith('data:')) updateData.image_url = og.image
          await db.from('products').update(updateData).eq('id', found.id)
          updated++
        } else {
          await db.from('products').insert({
            name: title, category, sub_category: '',
            sale_price: og.price || 0, original_price: 0,
            stock: 10, notes: '', image_url: og.image,
            store_url: url, alert_enabled: true,
          })
          added++
        }
